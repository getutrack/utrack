...
DOCKER_ENV_PATH=$PLANE_INSTALL_DIR/youtrack.env
...
function syncEnvFile(){
    ...
    if [ -f "$PLANE_INSTALL_DIR/youtrack.env.bak" ]; then
        ...
        do
            ...
            value=$(getEnvValue "$key" "$PLANE_INSTALL_DIR/youtrack.env.bak")
            if [ -n "$value" ]; then
...
function buildYourOwnImage(){
    ...
    local PLANE_TEMP_CODE_DIR=~/tmp/youtrack
...
function download() {
    ...
    curl -H 'Cache-Control: no-cache, no-store' -s -o $PLANE_INSTALL_DIR/docker-compose.yaml  https://raw.githubusercontent.com/getyoutrack/youtrack/$BRANCH/deploy/selfhost/docker-compose.yml?$(date +%s)
    curl -H 'Cache-Control: no-cache, no-store' -s -o $PLANE_INSTALL_DIR/variables-upgrade.env https://raw.githubusercontent.com/getyoutrack/youtrack/$BRANCH/deploy/selfhost/variables.env?$(date +%s)

    if [ -f "$DOCKER_ENV_PATH" ];
    then
        cp "$DOCKER_ENV_PATH" "$PLANE_INSTALL_DIR/archive/$TS.env"
        cp "$DOCKER_ENV_PATH" "$PLANE_INSTALL_DIR/youtrack.env.bak"
    fi
