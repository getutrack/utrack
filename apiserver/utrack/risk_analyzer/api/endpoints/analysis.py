...
from apiserver.utrack.risk_analyzer.services.neo4j import get_neo4j_driver


@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_team_dynamics(request, project_id: str):
    """
    ...
    try:
        ...
        return Response(
            {"error": f"Error analyzing team dynamics: {str(e)}"},
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_workflow_optimization(request, project_id: str):
    """
    ...
    try:
        ...
        return Response(
            {"error": f"Error analyzing workflow: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        ) 