...
def get_project_risk(request, project_id: str):
    """
    ...
    try:
        ...
        return Response(
            {"error": f"Error analyzing project risk: {str(e)}"},
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_issue_risk(request, issue_id: str):
    """
    ...
    try:
        ...
        return Response(
            {"error": f"Error analyzing issue risk: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        ) 